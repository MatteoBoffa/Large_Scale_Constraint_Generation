...
from core.rule_checker.baseline_RAG_checker import DenseForbiddenRAG
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="BAAI/bge-m3",
        help="Embedder model to compute RAG",
    )
    parser.add_argument(
        "--top_k",
        type=int,
        default=20,
        help="Number of elements retrieved by top K algorithm.",
    )
    parser.add_argument(
        "--reranker_model",
        type=str,
        default=None,
        help="Reranker model to sort RAG results",
    )
    parser.add_argument(
        "--hf_cache",
        type=str,
        default=None,
        help="Path for Huggingface cache. Default if None.",
    )
    parser.add_argument(
        "--prompt_path", type=str, default=None, help="Prompt to pre-pend to concepts."
    )
...
if __name__ == "__main__":
    ...
    rag = DenseForbiddenRAG(
        embed_model=args.embedder_model,
    ...
    for sample in tqdm(data, desc="Querying on premise model..."):
        ...
        top_k_concepts = rag.topk_from_pool(
            sentence, forbidden_pool=sample["concepts"], top_k=args.top_k
        )

        concepts = "[" + ", ".join([el.text for el in top_k_concepts]) + "]"
