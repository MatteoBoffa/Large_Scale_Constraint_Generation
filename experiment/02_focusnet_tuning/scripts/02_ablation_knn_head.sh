...
# Iterate over the different H_OUTS
    for H_OUT in "${H_OUTS[@]}"; do
        ...
        for FOLD in "${FOLDS[@]}"; do
            ...
            ########################################################################################## PHASE I - ENCODING
            #################### IO
            TRAINING_DATA_PATH="$REPO_ROOT/offline_datasets/processed_dataset/CommonNet/kfold_division/fold_$FOLD/train.parquet"
            VALIDATION_DATA_PATH="$REPO_ROOT/offline_datasets/processed_dataset/CommonNet/kfold_division/fold_$FOLD/validation.parquet"
            ...
            EPOCHS=30
            NS=(1) # Number of negative samples fixed to 1
            H_IN=768
            LRS=(2.5e-4)
            TEMPERATURES=(0.05)
            BATCH_SIZE=384
        ...
        #                          --temperature "$best_temperature" --loss_rules "$LOSS_RULES" --classifier "$CLASSIFIER"
