...
EXPERIMENT_ID="knn_training_hout_128_loss_rules_True"
GROUP="common_net"
...
H_OUT=128
H_IN=768    
CLASSIFIER="knn"  # Options: knn, random_forest
TEMPERATURE=0.05
...
CUDA_VISIBLE_DEVICES=$GPU_ID "$PY" ./test.py --experiment_id="$EXPERIMENT_ID" --seed="$SEED" \
                        ...
                        --normalize_embeddings="$NORMALIZE_EMBEDDINGS" --temperature "$TEMPERATURE" --batch_size="$BATCH_SIZE" \
