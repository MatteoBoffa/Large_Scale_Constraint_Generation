...
EXPERIMENT_ID="single_rule_random_forest_training_hout_128_loss_rules_True"
GROUP="common_net"
PROJECT="rule_constrainer"

### OUTPUT DATA
OUTPUT_PATH="$REPO_ROOT/experiment_results/$PROJECT/$GROUP/$EXPERIMENT_ID/test_results/wordnet_definitions/generic_definitions"
# Make sure we have clean logs for each run
[ -d "$OUTPUT_PATH" ] && rm -r "$OUTPUT_PATH"
mkdir -p $OUTPUT_PATH
### INPUT DATA
TEST_DATA_PATH="$REPO_ROOT/offline_datasets/processed_dataset/WordNet/rules_vs_definitions/generic_concepts_vs_definitions.json"
### MODEL
